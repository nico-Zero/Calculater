        ...
        try:
            match __check:
                case "+":
                    if call_v > 4:
                        try:
                            e1.insert(0,"=" + str((int(now_e1) + int(num_1))))
                        except ValueError:
                            e1.insert(0,"=" + str((float(now_e1) + float(num_1))))
                    elif call_v >= 2 and call_v <= 4:
                        e1.insert(0,"=" + str((int(num_1) + int(num_2))))
                case "-":
                    if call_v > 4:
                        try:
                            e1.insert(0,"=" + str((int(now_e1) - int(num_1))))
                ...
                case "%":   
                    if call_v > 4:
                        try:
                            e1.insert(0,"=" + str((int(now_e1) / 100)))
                        except ValueError:
                            e1.insert(0,"=" + str((float(now_e1) / 100)))
                    elif call_v >= 2 and call_v <= 4:
                        e1.insert(0,"=" + str((int(num_1)  / 100 )))
...
button_perc = Button(root, text="%", padx=48, pady=20, command= lambda: Perc("%"))
